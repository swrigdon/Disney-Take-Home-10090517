...
// Keep track of latest timeout id to clear
var timeout = null;
...
function appendBuffer(segment, arrayBuffer) {
...
function streamMovie(playlistUrl) {
    loadAndParsePlaylist(playlistUrl, function (segmentList) {
        ...
        function loadAndBufferSegment() {
            ...
            if (!segmentList[currentSegmentIndex]) {
                console.log('Done. No more MP4 segments to play');
                return;
            }

            loadSegment(segmentList[currentSegmentIndex].url, function (arrayBuffer) {

                appendBuffer(segmentList[currentSegmentIndex], arrayBuffer);

                currentSegmentIndex++

                // Load a new segment every 2 seconds for this test.
                timeout = setTimeout(loadAndBufferSegment, 2000);
            });
        }
